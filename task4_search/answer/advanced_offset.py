...
def offset_polygon(polygon, distances):
    ...
    area = sum((coords[i][0] * coords[(i+1) % len(coords)][1] - 
                coords[(i+1) % len(coords)][0] * coords[i][1]) for i in range(len(coords)))
...
def visualize(original, new, distances):
    ...
    for i, d in enumerate(distances):
        ...
        ax.text(mid[0], mid[1], f'{d:.2f}', ha='center', va='center',
    ...
    plt.savefig(f"result/{timestamp}/offset.png", dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    points = [(np.random.uniform(2,5)*np.cos(a), np.random.uniform(2,5)*np.sin(a)) for a in angles]
    poly = Polygon(points)
